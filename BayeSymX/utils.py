...
sq = {"name": "sq", "type": 1}
cu = {"name": "cu", "type": 1}
sqrt_op = {"name": "sqrt", "type": 1}

opset = [add, mul, neg, inv, sin, cos, exp, sq, cu, sqrt_op]
...
class BayeSymX_MCMC:
    def __init__(self, X, y, K, maxdepth, prior_params=None, add_intercept=True, wts_init=None, wts_prop=None, opset=None, ftset=None):
        ...
        if opset is None:
            self.opset = opset = [add, mul, neg, inv, sin, cos, exp, sq, cu, sqrt_op]
        else:
            self.opset = opset
    ...
    def plot_parallel_log_jmp(
        ...
        """
        Plot log-JMP traces from all parallel MCMC chains.
        ...
        alpha
            Transparency of each trace.

        linewidth
            Width of each trace.

        show
            If True, display the trace plot.

        save
            If True, save the trace plot.

        save_path
            Optional output directory or complete file path.
            ...
            Examples:
                ...
                save_path="results/plots/traces.png"
        ...
        if not hasattr(self, "parallel_result"):
            raise ValueError(
                "Run run_parallel_chains() before plotting "
                "parallel-chain traces."
            )
        ...
        if len(chains) == 0:
            raise ValueError(
                "No parallel-chain results were found."
            )
        ...
        for chain_idx, chain_result in enumerate(chains):
            kept_iterations = np.asarray(
                chain_result["kept_iterations"],
                dtype=int,
            )

            log_jmp = np.asarray(
                chain_result["log_jmp"],
                dtype=float,
            )

            seed = chain_result.get("seed")
        ...
        if save:
            ...
            if save_path is None:
                ...
                # Treat an existing directory or a path
                # without a file extension as a directory.
                if (
                    (
...
def reduce_topk_jmp_forests(
    ...
    prior_variance=10.0,
    rcond=None,
    return_candidate_tables=True,
):
    """
    Post-process top-k JMP forests by subset model reduction.

    For each top JMP forest with K symbolic trees, this enumerates all
    2^(K+1) component models when add_intercept=True:
        {intercept, tree_1, ..., tree_K}

    Each candidate subset is fitted by OLS using np.linalg.lstsq, so
    collinearity is handled through the SVD/minimum-norm least-squares
    solution. AIC/BIC uses the numerical rank as the effective number
    of regression coefficients.

    After the best subset is selected, coefficients are reported as the
    posterior mean from Bayesian linear regression on the selected design.
    ...
    top_k:
        Number of top JMP rows to process. Used if dataframes are omitted,
        or to truncate supplied dataframes.
    X, y:
        Dataset used for post-processing. Defaults to mcmc_obj.X, mcmc_obj.y.
    criterion:
        "BIC" or "AIC".
    force_intercept:
        If False, enumerates all 2^(K+1) subsets including models without
        intercept. If True, only considers subsets that include intercept.
    blr_prior:
        Optional tuple (beta0_full, V0_full, a0, b0) for the full component
        vector. If None, uses mcmc_obj beta prior when dimensions match.
    prior_variance:
        Fallback prior variance for beta if no compatible prior is found.
    rcond:
        Passed to np.linalg.lstsq / np.linalg.pinv.
    return_candidate_tables:
        Whether to return all candidate subset scores for each forest.

    Returns
    -------
    reduced_df:
        One row per top JMP forest, with selected components and posterior
        mean coefficients.
    details:
        List of dictionaries with forest objects, selected indices,
        candidate tables, fitted values, etc.
    """
    ...
    n = X.shape[0]
    add_intercept = bool(mcmc_obj.add_intercept)

    if top_expr_df is None or top_beta_df is None:
        top_expr_df, top_beta_df = mcmc_obj.summarize_parallel_topk_forests(top_k=top_k)

    if top_k is not None:
        top_expr_df = top_expr_df.head(top_k).copy()
        top_beta_df = top_beta_df.head(top_k).copy()

    def _get_forest_from_top_row(row):
        chain_id = int(row["chain_id"])
        iteration = int(row["iteration"])

        chain_res = mcmc_obj.parallel_result["chains"][chain_id - 1]
        kept_iterations = np.asarray(chain_res["kept_iterations"], dtype=int)

        match = np.where(kept_iterations == iteration)[0]
        if len(match) == 0:
            raise ValueError(
                f"Could not find forest for chain_id={chain_id}, iteration={iteration}."
            )

        return chain_res["forests"][int(match[0])]

    def _design_from_forest(forest):
        K = forest.ntrees
        TX_tree = np.zeros((n, K), dtype=float)

        for j in range(K):
            TX_tree[:, j] = forest._eval_node(0, j, X)

        if not np.all(np.isfinite(TX_tree)):
            raise ValueError("A symbolic tree produced non-finite values.")

        exprs = forest.expression_strings()

        if add_intercept:
            TX_full = np.hstack([np.ones((n, 1)), TX_tree])
            names = ["intercept"] + [f"tree_{j + 1}" for j in range(K)]
            expressions = ["1"] + exprs
        else:
            TX_full = TX_tree
            names = [f"tree_{j + 1}" for j in range(K)]
            expressions = exprs

        return TX_full, names, expressions
    ...
    def _ols_fit(D):
        if D.shape[1] == 0:
            ...
            rank = 0
            rss = float(np.dot(y - yhat, y - yhat))
            return beta_hat, yhat, rss, rank

        beta_hat, _, rank, _ = np.linalg.lstsq(D, y, rcond=rcond)
        yhat = D @ beta_hat
        rss = float(np.dot(y - yhat, y - yhat))
        return beta_hat, yhat, rss, int(rank)

    def _information_criteria(rss, rank):
        rss_safe = max(float(rss), np.finfo(float).tiny)
        aic = n * np.log(rss_safe / n) + 2.0 * rank
        bic = n * np.log(rss_safe / n) + np.log(n) * rank
        return aic, bic
    ...
    def _full_blr_prior(p_full):
        if blr_prior is not None:
            beta0_full, V0_full, a0, b0 = blr_prior
            beta0_full = np.asarray(beta0_full, dtype=float).reshape(-1)
            V0_full = np.asarray(V0_full, dtype=float)
        elif hasattr(mcmc_obj, "beta0") and hasattr(mcmc_obj, "V0"):
            beta0_full = np.asarray(mcmc_obj.beta0, dtype=float).reshape(-1)
            V0_full = np.asarray(mcmc_obj.V0, dtype=float)
            a0 = float(mcmc_obj.a0)
            b0 = float(mcmc_obj.b0)
        else:
            beta0_full = np.zeros(p_full)
            V0_full = prior_variance * np.eye(p_full)
            a0 = 0.05
            b0 = 0.05

        if beta0_full.shape != (p_full,) or V0_full.shape != (p_full, p_full):
            beta0_full = np.zeros(p_full)
            V0_full = prior_variance * np.eye(p_full)
            a0 = 0.05
            b0 = 0.05

        return beta0_full, V0_full, float(a0), float(b0)

    def _blr_posterior_mean(D, beta0, V0, a0, b0):
        p = D.shape[1]
        ...
        V0_inv = _inv_or_pinv(V0)
        Vn_inv = V0_inv + D.T @ D
        rhs = V0_inv @ beta0 + D.T @ y

        beta_n = _solve_or_pinv(Vn_inv, rhs)
        Vn = _inv_or_pinv(Vn_inv)
        a_n = a0 + n
        b_n = b0 + float(y @ y + beta0 @ V0_inv @ beta0 - beta_n @ Vn_inv @ beta_n)
    ...
    for pos, (_, top_row) in enumerate(top_beta_df.iterrows()):
        forest = _get_forest_from_top_row(top_row)
        TX_full, component_names, component_exprs = _design_from_forest(forest)

        p_full = TX_full.shape[1]
        beta0_full, V0_full, a0, b0 = _full_blr_prior(p_full)

        candidate_rows = []

        for mask in range(2 ** p_full):
            included = [j for j in range(p_full) if (mask >> j) & 1]

            if force_intercept and add_intercept and 0 not in included:
                continue

            D = TX_full[:, included] if included else np.zeros((n, 0))
            _, _, rss, rank_eff = _ols_fit(D)
            aic, bic = _information_criteria(rss, rank_eff)

            candidate_rows.append({
                "mask": int(mask),
                "included_indices": tuple(included),
                "included_components": tuple(component_names[j] for j in included),
                "n_raw_components": len(included),
        ...
        candidate_df = pd.DataFrame(candidate_rows).sort_values(
            by=[criterion, "rank_eff", "n_raw_components", "rss"],
            ascending=[True, True, True, True],
        ).reset_index(drop=True)

        best = candidate_df.iloc[0]
        selected_idx = list(best["included_indices"])
        D_sel = TX_full[:, selected_idx] if selected_idx else np.zeros((n, 0))

        beta_ols_sel, yhat_ols, rss_ols, rank_eff = _ols_fit(D_sel)

        beta0_sel = beta0_full[selected_idx] if selected_idx else np.zeros(0)
        V0_sel = (
            V0_full[np.ix_(selected_idx, selected_idx)]
            if selected_idx
            else np.zeros((0, 0))
        )

        beta_post_sel, Vn_sel, a_n, b_n = _blr_posterior_mean(
            D_sel, beta0_sel, V0_sel, a0, b0
        )

        yhat_post = D_sel @ beta_post_sel if selected_idx else np.zeros_like(y)
        post_rmse = float(np.sqrt(np.mean((y - yhat_post) ** 2)))

        beta_post_full = np.zeros(p_full)
        beta_ols_full = np.zeros(p_full)

        for local_pos, full_j in enumerate(selected_idx):
            beta_post_full[full_j] = beta_post_sel[local_pos]
            beta_ols_full[full_j] = beta_ols_sel[local_pos]

        out = {
            "jmp_rank": int(top_row["rank"]) if "rank" in top_row else pos + 1,
            "chain_id": int(top_row["chain_id"]),
            "chain_seed": int(top_row["chain_seed"]),
            "iteration": int(top_row["iteration"]),
            "original_log_JMP": float(top_row["log_JMP"]),
            "original_RMSE": float(top_row["RMSE"]) if "RMSE" in top_row else np.nan,
            "criterion": criterion.upper(),
            "best_AIC": float(best["aic"]),
            "best_BIC": float(best["bic"]),
            "best_score": float(best[criterion]),
            "candidate_models_checked": int(len(candidate_df)),
            "selected_rank_eff": int(rank_eff),
            "n_selected_components": int(len(selected_idx)),
            "selected_components": ", ".join(component_names[j] for j in selected_idx),
            "ols_RMSE_selected": float(np.sqrt(rss_ols / n)),
            "posterior_RMSE_selected": post_rmse,
        }

        for j, name in enumerate(component_names):
            out[f"selected_{name}"] = bool(j in selected_idx)
            out[f"post_mean_{name}"] = float(beta_post_full[j])
            out[f"ols_coef_{name}"] = float(beta_ols_full[j])
            out[f"expr_{name}"] = component_exprs[j]
        ...
        details.append({
            ...
            "component_expressions": component_exprs,
            "selected_indices": selected_idx,
            "selected_components": [component_names[j] for j in selected_idx],
            "selected_expressions": [component_exprs[j] for j in selected_idx],
            "beta_post_mean_selected": beta_post_sel,
            "beta_post_mean_full_with_zeros": beta_post_full,
            "beta_ols_selected": beta_ols_sel,
            "beta_ols_full_with_zeros": beta_ols_full,
            "posterior_params_selected": {
                "beta_n": beta_post_sel,
                "Vn": Vn_sel,
                "a_n": a_n,
                "b_n": b_n,
            },
            "yhat_posterior": yhat_post,
            "yhat_ols": yhat_ols,
            "candidate_models": candidate_df if return_candidate_tables else None,
        })

    reduced_df = pd.DataFrame(output_rows)
    return reduced_df, details

def reduced_model_to_string(
    row,
    coef_prefix="post_mean_",
    tol=1e-10,
    digits=4,
    use_symbolic_expressions=True,
):
    """
    Convert one row of reduced_df into a readable model string, dropping
    zero / unselected coefficients.

    Example output:
        1.2345 + 2.01*(x0) - 0.75*(sin(x1))
    """

    terms = []

    coef_cols = [c for c in row.index if c.startswith(coef_prefix)]

    # Put intercept first, then tree terms in numerical order
    def sort_key(col):
        name = col.replace(coef_prefix, "")
        if name == "intercept":
            return -1
    ...
    coef_cols = sorted(coef_cols, key=sort_key)

    for coef_col in coef_cols:
        component = coef_col.replace(coef_prefix, "")
        coef = float(row[coef_col])

        if abs(coef) <= tol:
            continue

        selected_col = f"selected_{component}"
        if selected_col in row.index and not bool(row[selected_col]):
            continue

        if component == "intercept":
            term_body = f"{abs(coef):.{digits}g}"
        else:
            if use_symbolic_expressions:
                expr_col = f"expr_{component}"
                expr = str(row[expr_col]) if expr_col in row.index else component
            else:
                # Displays tree_1 as f_1(x), tree_2 as f_2(x), etc.
                if component.startswith("tree_"):
                    j = component.split("_")[1]
                    expr = f"f_{j}(x)"
                else:
                    expr = component

            term_body = f"{abs(coef):.{digits}g}*({expr})"

        if not terms:
            terms.append(term_body if coef >= 0 else f"-{term_body}")
        else:
            sign = "+" if coef >= 0 else "-"
...
def simplify_symbolic_expr(expr_string, feature_cols, digits=6):
    """
    Simplify one symbolic tree expression string using sympy.

    Handles expressions produced by Forest.expression_strings(), including:
        ^ powers, sin(...), cos(...), exp(...), nested parentheses.
    """
...
def reduced_model_to_simplified_string(
    row,
    feature_cols,
    coef_prefix="post_mean_",
    tol=1e-10,
    ...
    """
    Build a readable reduced model string with zero coefficients dropped
    and each symbolic tree expression simplified by sympy.
    """
    ...
    for coef_col in coef_cols:
        ...
        if component == "intercept":
            ...
            expr_col = f"expr_{component}"
            raw_expr = str(row[expr_col])

            try:
                simp_expr = simplify_symbolic_expr(
                    raw_expr,
                    feature_cols=feature_cols,
                    digits=digits,
                )
            except Exception:
                # Fallback if sympy cannot parse some generated expression
                simp_expr = raw_expr

            term_body = f"{abs(coef):.{digits}g}*({simp_expr})"
